...
class TelegramClient:
    ...
    async def close(self) -> None:
        ...
        self._closed = True
        # Flush buffered albums BEFORE cancelling the polling task: a member
        # sitting in the debounce window is a message the user already sent, so
        # dropping it on shutdown would lose real input.
    ...
    def _flush_all_albums(self) -> None:
        """Flush every buffered album (used on shutdown so none are dropped)."""
