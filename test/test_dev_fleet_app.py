...
@pytest.mark.asyncio
async def test_force_remove_fresh_merged_allows_dirty():
    """Fresh-merged control: cached MERGED + fresh verdict confirms MERGED +
    dirty → force removal still proceeds (force keeps its merged-dirty
    meaning when freshly verified)."""
    import kiro_crew.apps.builtins.dev_fleet.server as mod
...
@pytest.mark.asyncio
async def test_toctou_clean_unmerged_force_omits_git_force():
...
@pytest.mark.asyncio
async def test_toctou_merged_verified_force_keeps_git_force():
    """Control: when force=True and the PR IS merged (freshly verified),
    the removal command MUST include --force (force keeps its meaning for
    merged-dirty overrides where the work is shipped)."""
    import kiro_crew.apps.builtins.dev_fleet.server as mod
    ...
    removal_cmds = [c for c in captured_cmds if "worktree" in c and "remove" in c]
    assert len(removal_cmds) == 1
    assert "--force" in removal_cmds[0], (
        "merged-verified force path must include --force in the git command"
    )
...
async def test_containment_allows_when_contained():
    """Containment control: cached MERGED + fresh head + branch OID IS
    contained in fresh head → force removal proceeds (work IS shipped)."""
    import kiro_crew.apps.builtins.dev_fleet.server as mod
