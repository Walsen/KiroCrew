...
class TestAlbumShutdown:
    @pytest.mark.asyncio
    async def test_close_flushes_buffered_album(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """A member inside the window is real user input -- never dropped.

        The idle window is set far longer than this test's patience, so the
        natural timer CANNOT be what delivers the message: only ``close()``
        flushing can. (With a short window the timer fires during the drain and
        the test passes even with the flush removed -- verified by mutation.)
        """
        ...
        assert len(received) == 1, "close() must flush buffered albums"
